...
#[post("/")]
pub async fn post_link<'a>(pool: web::Data<Pool>, json: web::Json<UrlRequest>) -> impl Responder {
...
fn add_link_db(full_url: &String, pool: web::Data<Pool>) -> Result<Url, diesel::result::Error> {
    let conn = &mut pool.get().unwrap();
    let new_url = Url {
        url: full_url.clone(),
